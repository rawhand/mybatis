 ...
 *
 * @param <E> 枚举类型，要继承ByteEnum
 */
public abstract class BaseByteEnumTypeHandler<E extends Enum<?> & ByteEnum> extends BaseTypeHandler<E> {

	protected Class<E> type;

	protected final E[] enums;

	public BaseByteEnumTypeHandler(Class<E> type) { // 若没有在本handler类上加注解MappedJdbcTypes和MappedTypes，则ResultMap中需要指定类型。
		if (type == null) {
		...
		if (this.enums == null) {
			throw new IllegalArgumentException(type.getSimpleName() + " can not parse to ByteEnum");
		}
	}
